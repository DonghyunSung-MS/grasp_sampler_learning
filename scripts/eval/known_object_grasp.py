...
def main():
    ...
    for seed, model_ckpt in enumerate(seeds_model_ckpt):
        ...
        for idx, object_name in enumerate(object_names):
            ...
            condition = torch.zeros(args.num_sample, len(object_names))
            condition[:, idx] = 1.0
            ...
            if method == "VAE" or method == "GAN":
                ...
                transrotvec = model.sample_grasp(args.num_sample, condition).detach().cpu().numpy()
            ...
            if args.q:
                ...
                for percent in [0.001, 0.01, 0.1, 0.5, 1.0]:
                    ...
                    data_row = [object_name, seed, num_samples, precision, coverage, exp_coverage]
    ...
    if args.q:
        quatitative_df = pd.DataFrame(quatitative_data, columns=["object_name","seed", "number of samples", "precision_rate (%)", "coverage_rate (%)", "exp_coverage"])
